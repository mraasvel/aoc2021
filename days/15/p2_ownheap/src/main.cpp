...
Data* allocateData(Point p, int score) {
...
int dijkstra(const vector<int>& grid, vector<int>& min_scores, Point end) {
	t_heap* heap = heap_init(&compareData);
	heap_insert(heap, allocateData(Point(0, 0), 0));
	while (heap->size > 0) {
		Data* ptr = (Data*)heap_pop(heap);
		const Data& val = *ptr;
		int score = val.score;
		...
		for (const auto& a : adj) {
			...
			if (ns < min_scores[index]) {
				heap_insert(heap, allocateData(a, ns));
				min_scores[index] = ns;
			}
		}
		free(ptr);
